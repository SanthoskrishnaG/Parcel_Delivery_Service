                ...
                <div class="card h-100 border-0 shadow-sm rounded-4">
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="fw-bold mb-0">${v.make}</h5>
                            <span class="badge ${statusColor} rounded-pill">${v.status}</span>
                        </div>
                        <p class="text-muted small mb-2"><i class="bi bi-hash me-1"></i> ID: ${v.id}</p>
                        <p class="text-muted small mb-2"><i class="bi bi-tag me-1"></i> Type: ${v.type}</p>
                        <p class="text-muted small mb-2"><i class="bi bi-card-text me-1"></i> Plate: ${v.plate}</p>
                        <p class="text-muted small mb-3"><i class="bi bi-person me-1"></i> Owner: ${v.owner}</p>
                        
                        ${v.status === 'In Delivery' ? `<p class="small text-primary fw-bold"><i class="bi bi-box-seam me-1"></i> Assigned: ${v.assignedParcel}</p>` : ''}
                        ...
                        <div class="mt-3 pt-3 border-top d-flex justify-content-between gap-2">
                            ...
                            ${v.owner === 'Rented' && v.status === 'Available' ? `<button class="btn btn-sm btn-outline-danger w-100" onclick="returnRental('${v.id}')">Return</button>` : ''}
                        </div>
                    </div>
                </div>
            </div>
