...
function renderFleetDashboard(filterStatus = 'All') {
    ...
    vehicles.forEach(v => {
        ...
        switch (v.status) {
            case 'AVAILABLE': statusColor = 'bg-success'; displayStatus = 'Available'; break;
            case 'ASSIGNED': statusColor = 'bg-primary'; displayStatus = 'Assigned'; break;
            case 'IN_DELIVERY': statusColor = 'bg-info text-dark'; displayStatus = 'In Delivery'; break;
            case 'MAINTENANCE': statusColor = 'bg-warning text-dark'; displayStatus = 'Maintenance'; break;
            case 'RENTED': statusColor = 'bg-dark'; displayStatus = 'Rented'; break;
            case 'RETURN_PENDING': statusColor = 'bg-danger'; displayStatus = 'Return Pending'; break;
        }
                ...
                <div class="card h-100 border-0 shadow-sm rounded-4">
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <span class="badge ${statusColor} rounded-pill">${displayStatus}</span>
                        </div>
                        ...
                        ${(v.status === 'IN_DELIVERY' || v.status === 'ASSIGNED') && v.currentParcel ? `<p class="small text-primary fw-bold"><i class="bi bi-box-seam me-1"></i> Assigned: ${v.currentParcel}</p>` : ''}
                        ...
                        <div class="mt-3 pt-3 border-top d-flex justify-content-between gap-2">
                            ...
                            ${v.ownership === 'Rented' && (v.status === 'AVAILABLE' || v.status === 'RETURN_PENDING') ? `<button class="btn btn-sm btn-outline-danger w-100" onclick="returnRental('${v.id}')">Return</button>` : ''}
                        </div>
                    </div>
                </div>
            </div>
...
function updateFleetStats(vehicles) {
    ...
    const inDelivery = vehicles.filter(v => v.status === 'IN_DELIVERY' || v.status === 'ASSIGNED').length;
