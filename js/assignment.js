...
function renderAssignmentPanel() {
    ...
    // Priority 1 & 2: Available Company vehicle, smallest suitable
    let companyVehicles = vehicles.filter(v => v.status === 'AVAILABLE' && v.ownership === 'Company' && v.capacity >= pWeight);
    companyVehicles.sort((a, b) => a.capacity - b.capacity); // Sort ascending by capacity
    
    shortageAlert.classList.add('d-none');
    
    if (companyVehicles.length > 0) {
        // We have suitable company vehicles
        ...
        panel.innerHTML = `
            <div class="text-start">
                ...
                <h6 class="fw-bold mb-3">Available Company Vehicles</h6>
        ...
        // Priority 3 & 4: Search rentals, smallest suitable
        ...
        if (recommendedRental) {
            ...
            // Priority 5: No suitable rental exists
            panel.innerHTML = `
                <div class="card border-danger border-opacity-25 shadow-sm rounded-4 mb-4">
                    <div class="card-body p-5 text-center">
                        <i class="bi bi-x-octagon-fill text-danger fs-1 mb-3 d-block"></i>
                        <h4 class="fw-bold text-danger mb-3">Delivery Cannot Be Assigned</h4>
                        <p class="text-muted mb-4 px-md-4">No suitable vehicle is currently available<br>in the company fleet or rental fleet.</p>
                        
                        <div class="row justify-content-center mb-4 text-start">
                            <div class="col-sm-8 col-md-6">
                                <div class="bg-light p-3 rounded-3 border text-center">
                                    <p class="small text-muted mb-1">Required Capacity:</p>
                                    <p class="fw-bold mb-0 text-danger fs-5">${pWeight} kg</p>
                                </div>
                            </div>
                        </div>
                        
                        <button class="btn btn-outline-secondary rounded-pill px-4 mt-2" onclick="document.getElementById('assignmentPanel').innerHTML = ''">Close</button>
                    </div>
                </div>
            `;
        }
